  ...
  switch (type) {
    ...
    case 'tags':
      if (document.querySelector('.main-title')) {
        parent = document.querySelector('.main-title')
        target = document.querySelectorAll('div.tag.is-info')
      }
      break;
    ...
    switch (type) {
      ...
      case 'tags':
        ...
        for (let i = 0; i < target.length; i++) {
          if (!['含磁鏈', '含字幕'].includes(target[i].innerText)) {
            textList.push(target[i].innerText)
          }
        }
        text = textList.join('、') || document.querySelector('.tabs .is-active').innerText
        ...
        data = `type=${type}&code=${code}&title=${text}${textList.length > 0 ? `（${document.querySelector('.tabs .is-active').innerText}）` : ''}&params=${encodeURIComponent(location.search)}&extend=${pathname[1] ? pathname[1] : ''}`
        break;
      default:
        if (target) {
